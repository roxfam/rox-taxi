...
- **[Feb 2026]** **Backend refactor** — `server.py` shrunk from 1677 → 1360 lines by extracting: catalog seeds → `/app/backend/seed_data.py` (TOURS_SEED, TAXI_SERVICES, RENTALS_SEED, CURRENT_RENTAL_IDS); PDF builders → `/app/backend/pdf_utils.py` (`build_wedding_pdf`, `build_receipt_pdf`). All routes and behaviour identical — verified by pytest (9/9 backend) + testing agent iteration 8.
