...
### ✅ Shipped Feb 2026 (Category email routing + SEO boost + tour photos + deploy prep)
- **Per-category email senders** — `send_email(..., category=...)` in `notifications.py` now routes to dedicated inboxes: `confirmation@roxtaxi.com` (all booking confirmations, reminders, rental-return reminders, paid receipts), `quotes@roxtaxi.com` (custom quote request replies), `info@roxtaxi.com` (contact-form + group-inquiry replies). Env vars `EMAIL_FROM_CONFIRMATION/QUOTES/INFO` drive it; also editable live via Admin → Tokens → Email. `.env.example` updated. Backend restarted clean, contact POST returned 200 with new routing.
- **SEO overhaul for "Nassau taxi service Bahamas"** — keyword-focused `<title>` + meta description, 45+ long-tail keywords, richer OG + Twitter Card, expanded JSON-LD (LocalBusiness with `keywords`/`slogan`/`knowsAbout`/`makesOffer`/2× `Review`, new TaxiService node, BreadcrumbList, 11-question FAQ). Sitemap rebuilt with `lastmod` + image sitemap + per-attraction pages.
