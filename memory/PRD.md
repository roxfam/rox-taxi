...
- **[Feb 2026]** **AdminDashboard Force-send button** — the notification cell on paid bookings now shows two buttons: the standard "Re-send" (respects toggles) and a new gold "Force" button (`data-testid="notify-force-resend-<id>"`) which calls `POST /api/admin/bookings/{id}/resend-notification` with `{force:true}`, bypassing the admin's email/SMS notification toggles for one-off manual sends.
