...
- **Referral rewards** ✅ (Feb 2026, this pass) — every new signup auto-gets a `ROX-XXXXXX` code + `credit_balance` field. Optional `referral_code` accepted in `POST /api/auth/register` (silently ignored if invalid to prevent enumeration). `_apply_referral_conversion_if_paid` hook in `payments._mark_paid` fires once per referee's first paid booking, awards **$25** every 5th conversion to the referrer's balance. `GET /api/referrals/summary` returns code + counts + progress. Signup form has referral input (auto-fills from `?ref=CODE`), MyBookings shows a referral card with copy-link + stats + progress bar to next reward.
