...
- **[Feb 2026]** **Compact car rental photo** — `spark-compact` (2019 Chevrolet Spark) updated to the crisp white Spark on white background from the user's asset library. Both `seed_data.py` (for future re-seeds) and the live `db.rentals` document have been updated.
