...
- **[Feb 2026]** **Iteration 10 review fixes** — SiteConfigPanel logo upload now flows through `/api/admin/images` (shared library), added `logo-pick-btn` opening the ImagePickerModal for logos, and Copy URL button gained a `document.execCommand("copy")` fallback for browsers where `navigator.clipboard` is blocked (iframe/insecure).
