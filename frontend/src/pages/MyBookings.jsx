  ...
  if (!user) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-24 text-center" data-testid="mybookings-signin">
        <div className="w-16 h-16 rounded-full bg-[#00B4D8]/10 text-[#00B4D8] mx-auto flex items-center justify-center mb-6">
          <Ticket className="w-7 h-7" />
        </div>
        <h1 className="serif text-4xl sm:text-5xl text-[#1A365D]">Sign in to see your rides.</h1>
        ...
        <button
          ...
          className="mt-8 inline-flex items-center gap-3 rounded-full bg-white border border-[#E2E8F0] px-6 py-3 text-sm font-semibold hover:border-[#00B4D8] active:scale-95"
        >
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="serif text-3xl text-[#1A365D]">Welcome back, {user.name?.split(" ")[0] || "friend"}.</div>
            <div className="text-sm text-[#64748B] mt-0.5">{user.email}</div>
          </div>
        </div>
        <button onClick={logout} className="inline-flex items-center gap-2 rounded-full border border-[#E2E8F0] px-4 py-2 text-sm hover:border-[#1A365D]" data-testid="mybookings-logout">
          <LogOut className="w-4 h-4" /> Sign out
        </button>
      </div>

      <h2 className="serif text-3xl text-[#1A365D] mt-12">Your bookings</h2>
      {bookings.length === 0 ? (
        <div className="mt-6 rounded-2xl border border-dashed border-[#E2E8F0] p-10 text-center text-[#64748B]">
          You haven't made a booking yet. <Link to="/taxi" className="text-[#00B4D8] hover:underline">Book a taxi →</Link>
        </div>
      ) : (
        <div className="mt-6 grid gap-4">
          {bookings.map((b) => {
            ...
            return (
              <div key={b.id} className="rounded-2xl border border-[#E2E8F0] bg-white p-6 flex flex-wrap gap-6 items-center justify-between hover:-translate-y-0.5 hover:shadow-[0_16px_40px_rgba(0,180,216,0.08)] transition-transform" data-testid={`mybookings-item-${b.id}`}>
                <div>
                  <div className="mono text-lg text-[#1A365D] font-semibold">{b.id}</div>
                  <div className="text-sm text-[#1A365D] mt-1">{b.item_name}</div>
                ...
                <div className="flex-1 min-w-[240px]">
                  ...
                  <div className="mt-1 text-sm font-semibold text-[#1A365D]">{STATUS_STEPS[Math.max(idx,0)]?.label || b.status.replace("_"," ")}</div>
                  <div className="mt-2 h-1 rounded-full bg-[#F1F5F9] overflow-hidden">
                    <div className="h-full bg-[#00B4D8]" style={{ width: `${Math.max(idx,0)/(STATUS_STEPS.length-1)*100}%` }} />
                  </div>
                </div>
                <div className="text-right">
                  <div className="mono text-lg text-[#FF7F50] font-semibold">{money(b.total)}</div>
                  <Link to={`/track?id=${b.id}`} className="mt-1 inline-flex items-center gap-1 text-xs text-[#00B4D8] hover:underline">Track <ArrowRight className="w-3 h-3" /></Link>
                </div>
              </div>
            );
          })}
