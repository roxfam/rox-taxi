  ...
  return (
    <div data-testid="rentals-page">
      <section className="relative py-24 bg-[#FAF9F6]">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 grid md:grid-cols-12 gap-8 items-end">
          <div className="md:col-span-7">
            ...
            <h1 className="serif text-6xl sm:text-7xl mt-3 leading-[0.9] text-[#1A365D]">Rent a car in <em className="italic text-[#00B4D8]">Nassau</em>.</h1>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-10 pb-24 grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {rentals.map((r) => (
          <div key={r.id} className="group rounded-2xl overflow-hidden bg-white border border-[#E2E8F0] hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(0,180,216,0.15)] transition-transform flex flex-col" data-testid={`rental-card-${r.id}`}>
...
function BlackoutList({ blackouts }) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3" data-testid="rental-blackouts">
      ...
      {upcoming.length === 0 ? (
        <div className="mt-1.5 text-xs text-[#00B4D8] flex items-center gap-1">
          <Info className="w-3 h-3" /> Available now — no upcoming bookings
        </div>
      ) : (
        <ul className="mt-1.5 space-y-0.5">
          {upcoming.map((b) => (
            <li key={b.booking_id} className="text-xs mono text-[#1A365D]">
